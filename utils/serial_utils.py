...
def generate_serial_number(pump_model, configuration, cursor):
    ...
    # ABCD components
    model_code = PUMP_MODEL_CODES[pump_model]  # A
    config_code = CONFIG_CODES[configuration]  # B
    static_c = "0"                            # C
    static_d = "1"                            # D
    abcd = f"{model_code}{config_code}{static_c}{static_d}"  # e.g., "5101"

    year = datetime.now().strftime("%y")  # HI, e.g., "25" for 2025
